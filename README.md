...
| --- | --- | --- |
| 簡潔に伝える | `reply_r1_plain_v2`（0909改訂版・感情言語化なし） | 通常の確認・進捗連絡 |
| 配慮を添える | `reply_r1_verbalize_v2`（0909改訂版・感情言語化あり） | お詫び・念のための確認・寄り添い |
| 配慮＋論点整理 | `reply_r2_empathy_organized`（案②-C 共感＋整理型） | 行き違い・未解決事項がある場面（検証②で最高評価） |
...
| データセット | 通数 | 内容 |
| --- | --- | --- |
| ① 関係性混在メール | 15 | 同じ相手との関係良好10通／関係険悪5通 |
| ② 混在感情メール | 50 | 丁寧だが婉曲な断り等（既存ツールと同一の `.eml`） |
...
ステージ1の最新指示文は「英単語＋（日本語）」のキーで返します。アプリ側は英語キーに正規化して扱います。
...
> **総合優先度は「AIの出力値」を採用値**とし、研究レポートの加重式は**参考値**として併記します。
...
動画・スクリーンショットを公開するときは `DEMO_ALIAS=1` を設定します。

```toml
DEMO_ALIAS = "1"
```

- 「使うAI」の表示が **LLM-A（高速）／LLM-B（高品質）／LLM-C** になり、モデル名・接続先URL・環境変数名・指示文IDを画面に出しません
- メールサービスの選択も「メールサービスA／B」になります
- **差し替えるのは表示だけ**で、呼び出すモデル・接続先・設定方法は何も変わりません（実名はこのREADMEと設計書に残ります）
- 実体の対応は `202609_最終成果物作成/003_動画用にアプリをブラッシュアップ/009_CLAUDE_商標名を伏せるデモ表示モード.md` を参照
