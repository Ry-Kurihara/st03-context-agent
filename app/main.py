...
def main() -> None:
    ...
    st.page_link(
        "pages/0_📬_受信トレイ.py",
        label="受信トレイを開く（優先度の読み取り → 返信案3択まで1画面で完結）",
        icon="📬",
        use_container_width=True,
    )
    st.caption("研究で有効性を確認したプロンプト（ステージ1：改良版／ステージ2：0909改訂版・共感＋整理型）を搭載しています。")

    st.subheader("🧪 研究の検証フロー（ステップごとに確認する）")
    nav = [
        ("pages/1_📥_メールデータ.py", "① メールデータを選ぶ", "📥"),
        ("pages/2_🔍_ステージ1_感情分析.py", "② ステージ1：感情分析を実行する", "🔍"),
        ("pages/4_✍️_ステージ2_返信案生成.py", "③ ステージ2：返信案A/Bを作る", "✍️"),
        ("pages/92_🔬_研究_返信案ペア比較.py", "④ 返信案A/Bを比べて記録する", "🔬"),
    ]
    ...
    with st.expander("研究用のそのほかの画面"):
        st.page_link("pages/91_🔬_研究_ステージ1プロンプト比較.py", label="ステージ1：プロンプト比較", icon="🔬")
        st.page_link("pages/93_🔬_研究_答え合わせ.py", label="答え合わせ（人 vs AI ／ ペア比較の集計）", icon="🔬")
        st.page_link("pages/94_🔬_研究_プロンプト管理.py", label="プロンプト管理（指示文を読む・保存する）", icon="🔬")
...
| 研究ステージ | やること | 使うページ |
| --- | --- | --- |
| **ステージ1** 感情パラメータ化 | 受信メールを読み解き、緊急度・不満度・優先度などを数値化する（ステージ1の最新指示文） | 🔍 ステージ1_感情分析 |
| **メールクライアント** | 受信メールを優先度順に並べ、選んだメールの返信案を3つの書き方で作る | 📬 受信トレイ |
| ステージ1（改良） | 分析プロンプトを2つ並べ、スコアの差を見る | 🔬 研究_ステージ1プロンプト比較 |
| **ステージ2** 返信案生成 | 「① パラメータ ＋ ② メール本文」をAIに渡し、**返信案A/Bを生成**する | ✍️ ステージ2_返信案生成 |
| ステージ2（評価） | 返信案A/Bを読み比べて、どちらが良いかと理由を記録する | 🔬 研究_返信案ペア比較 |
...
4. **🔬 研究_返信案ペア比較** … A/Bを読み比べ、どちらが良いか＋理由を記録（CSVでダウンロードできます）。
5. **🔬 研究_答え合わせ** … 溜めた記録の集計を見ます。
"""
    )
    ui.caution_box()
    ...
    with col2:
        ...
        st.caption("指示文の中身は「🔬 研究_プロンプト管理」で読めます／自分用に保存もできます。")
